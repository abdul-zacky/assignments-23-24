...
<<<<<<< HEAD
        ...
        if (userLoggedIn == null) {
            ...
            UserSystemCLI theUser = loginManager.getSystem(userLoggedIn.role);
            // While loop selama menu keluar tidak diinput
            while (true) {
                theUser.displayMenu();
                int command = Integer.parseInt(input.nextLine());
                // Panggil method handleMenu sesuai role user
                boolean contOrBreak = theUser.handleMenu(command, userLoggedIn);
                System.out.println("");
                if (!contOrBreak)
                    break;
            }
